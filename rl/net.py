...
class ActorPPO(nn.Module):
    def __init__(self, mid_dim, state_dim, action_dim):
        ...
        if isinstance(state_dim, int):
            ...
            self.net1 = nn.Sequential(nn.Linear(netLen, mid_dim), nn.ReLU(),
                                      nn.Linear(mid_dim, int(mid_dim / 2)), nn.ReLU(),
                                      nn.Linear(int(mid_dim / 2), int(mid_dim / 2 ** 2)), nn.ReLU(),
                                      nn.Linear(int(mid_dim / 2 ** 2), action_dim),
                                      # 归一化动作输出 [-1,1]
                                      nn.Tanh())
        ...
        layer_norm(self.net1[-2], std=0.1)  # output layer for action

    def forward(self, state):
        state = self.net0(state)
        return self.net1(state).tanh()  # action

    def get_action_noise(self, state):
        state = self.net0(state)
        a_avg = self.net1(state)
...
class CriticAdv(nn.Module):
    ...
    def forward(self, state):
        state = self.net0(state)
        return self.net1(state)  # Q value
