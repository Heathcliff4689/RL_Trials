...
def latterPartState(_channel_norm):
    ...
    :return:
    """
    channel_norm = deepcopy(_channel_norm)
...
def getFbb(F_rf, Pw, groups_dict):  # haven't been tested
    ...
    for item in groups_order_dict:
        t = groups_order_dict[item][0]  # 4 test
...
def getGroupGainTest(g, theta):
    group_gains = []
    for itr in g:
        gains = []
        for i in theta:
            f_rf1 = computeBfVector(i)
            gain1 = computeChABFGain(g[itr], f_rf1)
            gains.append(gain1)
