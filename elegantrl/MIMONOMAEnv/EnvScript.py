...
class MIMONOMAEnv:
    ...
    def __init__(self,
                 ...
                 random_seed=args_channel.seed, P_min=args_channel.P_min, P_max=args_channel.P_max):
    ...
    def step(self, action) -> (np.ndarray, float, bool, None):
        ...
        action_0 = np.clip(action[0], 0, 2 * np.pi)
        action_0 = self.actionNormal(action_0)

        self.theta[curr_group_index] = action_0
        self.Pw[curr_group_index] = np.clip(action[1], self.P_min, self.P_max)
